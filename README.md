...
**Modified (ML-based with Trial Positions):**
```python
def evaluate_board_raw(board: chess.Board, num_trials: int = 3) -> int:
    """🤖 MODIFIED: Now uses ML-based evaluation with trial positions"""
    ...
    if ML_AVAILABLE:
        ...
        with torch.no_grad():
            ...
            x_current = board_to_vec(board)
            current_score = ml_model(x_current).item()
            
            # 2. Try a few trial positions (best legal moves)
            trial_moves = sorted(legal_moves, key=priority)[:num_trials]
            
            # 3. Evaluate each trial position
            trial_scores = []
            for move in trial_moves:
                board.push(move)
                trial_score = ml_model(board_to_vec(board)).item()
                board.pop()
                trial_scores.append(trial_score)
            
            # 4. Blend current score with best trial score
            best_trial = max(trial_scores)
            final_score = 0.7 * current_score + 0.3 * best_trial
...
- ✅ **NEW: Evaluates multiple trial positions (default: 3)**
- ✅ **NEW: Prioritizes captures and checks for trial moves**
- ✅ **NEW: Blends current position with best continuation (70/30)**
